...
if (isset($_POST["sql_password"])) {
    ...
    if ($conn->connect_error) {
        ...
        $sql = "DROP DATABASE  if exists " . $_POST["sql_name"] . ";" . "create DATABASE " . $_POST["sql_name"] . ";";
        if ($conn->multi_query($sql) === TRUE) {
            echo "<h1 style='color: #66ccff;'>数据库创建成功，正在新建表格。。。</h1>";
            ...
            if ($is_ok === TRUE) {
                echo "<h1 style='color: #66ccff;'>表格创建成功，正在导入数据。。。</h1>";
                clear_cache($conn);
                $filename = "../data/" . $_POST["chose"] . ".sql";
                $handle = fopen($filename, "r");//读取二进制文件时，需要将第二个参数设置成'rb'
